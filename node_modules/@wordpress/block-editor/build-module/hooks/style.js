 ...
 */
import { first, forEach, get, has, isEmpty, isString, kebabCase, map, omit, startsWith } from 'lodash';
...
function compileElementsStyles(selector) {
  ...
  return map(elements, (styles, element) => {
    ...
    if (!isEmpty(elementStyles)) {
      ...
      return [`.editor-styles-wrapper .${selector} ${ELEMENTS[element]}{`, ...map(elementStyles, (value, property) => `\t${kebabCase(property)}: ${value};`), '}'].join('\n');
    }
...
const withElementsStyles = createHigherOrderComponent(BlockListBlock => props => {
  ...
  return createElement(Fragment, null, elements && element && createPortal(createElement("style", {
    ...
    className: elements ? classnames(props.className, blockElementsContainerIdentifier) : props.className
  })));
});
