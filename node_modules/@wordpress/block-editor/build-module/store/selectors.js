...
export const getBlocksByClientId = createSelector((state, clientIds) => map(castArray(clientIds), clientId => getBlock(state, clientId)), (state, clientIds) => map(castArray(clientIds), clientId => state.blocks.tree[clientId]));
/**
...
export const getBlockParentsByBlockName = createSelector(function (state, clientId, blockName) {
  ...
  return map(filter(map(parents, id => ({
    id,
...
export function hasSelectedInnerBlock(state, clientId) {
  ...
  return some(getBlockOrder(state, clientId), innerClientId => isBlockSelected(state, innerClientId) || isBlockMultiSelected(state, innerClientId) || deep && hasSelectedInnerBlock(state, innerClientId, deep));
}
/**
 * Returns true if the block corresponding to the specified client ID is
...
export function isAncestorBeingDragged(state, clientId) {
  ...
  return some(parents, parentClientId => isBlockBeingDragged(state, parentClientId));
}
/**
...
export const canInsertBlockType = createSelector(canInsertBlockTypeUnmemoized, (state, blockName, rootClientId) => [state.blockListSettings[rootClientId], state.blocks.byClientId[rootClientId], state.settings.allowedBlockTypes, state.settings.templateLock]);
/**
...
export const hasInserterItems = createSelector(function (state) {
  let rootClientId = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
  const hasBlockType = some(getBlockTypes(), blockType => canIncludeBlockTypeInInserter(state, blockType, rootClientId));
...
export const __experimentalGetAllowedBlocks = createSelector(function (state) {
  ...
  return filter(getBlockTypes(), blockType => canIncludeBlockTypeInInserter(state, blockType, rootClientId));
